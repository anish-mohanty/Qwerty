...
public class TransactionService {
    ...
    public Transaction processDirectDebit() {
        ...
        DirectDebitTransaction directDebitTransactionV1 =
        ...
        directDebitTransaction.setAmount(
                faker.number().numberBetween(10000, 100000));

        directDebitTransactionV1.setPaymentDate(
                faker.date().birthday()
                        .toInstant()
                        .atZone(ZoneId.systemDefault())
                        .toLocalDate());
    ...
    public Transaction processExternalDebit() {
        ...
        externalDebitTransaction.setAmount(
                faker.number().numberBetween(10000, 100000));

        externalDebitTransaction.setBranchAddress(
                faker.address().fullAddress());

        externalDebitTransaction.setBranchCode(
                faker.number().digits(5).toString());

        externalDebitTransaction.setBranchPostalCode(
                faker.address().zipCode());

        externalDebitTransaction.setBranchName(
                faker.company().name());
